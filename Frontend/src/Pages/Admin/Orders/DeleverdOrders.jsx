  ...
  if (DeliveredOrders.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-900">No Delivered Orders</h3>
...
const Pagination = ({ page, totalPages, totalItems, itemsPerPage, onPageChange }) => (
  <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
    <div className="text-sm text-gray-600">
      ...
      <span className="font-medium">{totalItems}</span> Delivered orders
    </div>
...
const DeliveredOrders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`/api/v1/admin/getallorders`, {
        params: { page, limit: ITEMS_PER_PAGE, status: 'delivered' },
        headers: {
  ...
  const changeOrderStatus = async (reason = "") => {
    ...
    try {
      ...
      console.log('Delivered order response:', response);
    ...
    <div className="min-h-screen p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-2xl md:text-3xl font-bold text-white-800">Delivered Orders</h1>
